...
本文记录网关、灰度治理和 Docker 编排的本地运行要点，便于小组联调和验收。
...
网关配置位于 `dismai-server/dismai-gateway-service/src/main/resources/application.yml` 和 `application-pro.yml`。
`application-pro.yml` 负责声明 Gateway 路由，容器环境通过 `docker-compose.yml` 覆盖 Nacos、Redis、Kafka 和 Sentinel 地址。

## 灰度治理

灰度路由使用请求头 `gray` 控制：

- `gray: true` 优先选择 Nacos 元数据为 `gray=true` 的服务实例。
- `gray: false` 或不传该请求头时，默认选择非灰度实例。
- 请求灰度实例但注册中心没有灰度实例时，网关回退到普通实例，保证联调可用。
