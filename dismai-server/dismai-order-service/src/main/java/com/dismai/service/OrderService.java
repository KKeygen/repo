...
import com.dismai.client.PayClient;
...
import com.dismai.dto.OrderCancelDto;
import com.dismai.dto.OrderCreateDto;
import com.dismai.dto.OrderGetDto;
import com.dismai.dto.OrderListDto;
import com.dismai.dto.OrderPayCheckDto;
import com.dismai.dto.OrderPayDto;
...
import com.dismai.enums.PayChannel;
...
import com.dismai.service.delaysend.DelayOperateProgramDataSend;
import com.dismai.service.properties.OrderProperties;
...
import com.dismai.vo.OrderGetVo;
import com.dismai.vo.OrderListVo;
import com.dismai.vo.OrderPayCheckVo;
import com.dismai.vo.OrderTicketInfoVo;
import com.dismai.vo.SeatVo;
import com.dismai.vo.TicketUserInfoVo;
import com.dismai.vo.TicketUserVo;
import com.dismai.vo.TradeCheckVo;
import com.dismai.vo.UserAndTicketUserInfoVo;
import com.dismai.vo.UserGetAndTicketUserListVo;
import com.dismai.vo.UserInfoVo;
...
import static com.dismai.core.DistributedLockConstants.UPDATE_ORDER_STATUS_LOCK;
import static com.dismai.core.RepeatExecuteLimitConstants.CANCEL_PROGRAM_ORDER;
import static com.dismai.core.RepeatExecuteLimitConstants.CREATE_PROGRAM_ORDER_MQ;
import static com.dismai.core.RepeatExecuteLimitConstants.PROGRAM_CACHE_REVERSE_MQ;
...
public class OrderService extends ServiceImpl<OrderMapper, Order> {
    ...
    public String create(OrderCreateDto orderCreateDto) {
        ...
        if (Objects.nonNull(oldOrder)) {
            throw new DismaiFrameException(BaseCode.ORDER_EXIST);
        }
    ...
    public String pay(OrderPayDto orderPayDto) {
        ...
        if (Objects.isNull(order)) {
            throw new DismaiFrameException(BaseCode.ORDER_NOT_EXIST);
        }
        if (Objects.equals(order.getOrderStatus(), OrderStatus.CANCEL.getCode())) {
            throw new DismaiFrameException(BaseCode.ORDER_CANCEL);
        }
        if (Objects.equals(order.getOrderStatus(), OrderStatus.PAY.getCode())) {
            throw new DismaiFrameException(BaseCode.ORDER_PAY);
        }
        if (Objects.equals(order.getOrderStatus(), OrderStatus.REFUND.getCode())) {
            throw new DismaiFrameException(BaseCode.ORDER_REFUND);
        }
        if (orderPayDto.getPrice().compareTo(order.getOrderPrice()) != 0) {
            throw new DismaiFrameException(BaseCode.PAY_PRICE_NOT_EQUAL_ORDER_PRICE);
        }
        ...
        if (!Objects.equals(payResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new DismaiFrameException(payResponse);
        }
    ...
    public OrderPayCheckVo payCheck(OrderPayCheckDto orderPayCheckDto){
        ...
        tradeCheckDto.setChannel(Optional.ofNullable(PayChannel.getRc(orderPayCheckDto.getPayChannelType()))
                .map(PayChannel::getValue).orElseThrow(() -> new DismaiFrameException(BaseCode.PAY_CHANNEL_NOT_EXIST)));
        ...
        if (!Objects.equals(tradeCheckVoApiResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new DismaiFrameException(tradeCheckVoApiResponse);
        }
        TradeCheckVo tradeCheckVo = Optional.ofNullable(tradeCheckVoApiResponse.getData())
                .orElseThrow(() -> new DismaiFrameException(BaseCode.PAY_BILL_NOT_EXIST));
        if (tradeCheckVo.isSuccess()) {
            ...
            throw new DismaiFrameException(BaseCode.PAY_TRADE_CHECK_ERROR);
        }
        return orderPayCheckVo;
    }
    ...
    public void updateOrderRelatedData(Long orderNumber,OrderStatus orderStatus){
        ...
        if (updateOrderResult <= 0 || updateTicketUserOrderResult <= 0) {
            throw new DismaiFrameException(BaseCode.ORDER_CANAL_ERROR);
        }
        ...
        if (CollectionUtil.isEmpty(orderTicketUserList)) {
            throw new DismaiFrameException(BaseCode.TICKET_USER_ORDER_NOT_EXIST);
        }
        if (Objects.equals(orderStatus.getCode(), OrderStatus.CANCEL.getCode())) {
            redisCache.incrBy(RedisKeyBuild.createRedisKey(
    ...
    public void updateProgramRelatedDataResolution(Long programId,Map<Long,List<Long>> seatMap,OrderStatus orderStatus,
        ...
        data[3] = JSON.toJSONString(Optional.ofNullable(idNumberList).orElse(new ArrayList<>()));
    ...
    public OrderGetVo get(OrderGetDto orderGetDto) {
        ...
        if (!Objects.equals(userGetAndTicketUserApiResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new DismaiFrameException(userGetAndTicketUserApiResponse);

        }
        UserGetAndTicketUserListVo userAndTicketUserListVo =
                Optional.ofNullable(userGetAndTicketUserApiResponse.getData())
                        .orElseThrow(() -> new DismaiFrameException(BaseCode.RPC_RESULT_DATA_EMPTY));
    ...
    public boolean initiateCancel(OrderCancelDto orderCancelDto){
        ...
        if (!Objects.equals(order.getOrderStatus(), OrderStatus.NO_PAY.getCode())) {
            throw new DismaiFrameException(BaseCode.CAN_NOT_CANCEL);
        }
